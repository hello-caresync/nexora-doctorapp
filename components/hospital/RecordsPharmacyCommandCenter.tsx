      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
            Records &amp; Pharmacy Command Center · Node: {nodeId} ({hospitalName})
          </div>
          <h3 className="mt-1 text-lg font-black text-slate-900">Records &amp; Pharmacy</h3>
          <p className="text-xs text-slate-500">
            Patient medical records, prescriptions and pharmacy operations in one workspace.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-1 text-[10px] font-black uppercase text-emerald-700">
            <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />
            Live Sync
          </span>
          <span className="text-[10px] font-medium text-slate-500">
            Last updated: {lastUpdated ? formatClock(lastUpdated.toISOString()) : '—'}
          </span>
          <button
            type="button"
            onClick={() => void loadAll()}
            className="inline-flex items-center gap-1.5 rounded-xl bg-cyan-700 px-3 py-2 text-xs font-bold text-white"
          >
